...
export default function LoginScreen() {
  ...
  // Inyectar Google Identity Services script en web
  useEffect(() => {
      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.card}>
          ...
          {isRegister && (
            <>
              <TouchableOpacity style={s.avatarPicker} onPress={pickAvatar}>
                ...
                {avatar && <View style={s.avatarEditBadge}><Text style={{ fontSize: 10 }}>✏️</Text></View>}
              </TouchableOpacity>

              <View style={s.field}>
                <Text style={s.label}>USERNAME</Text>
                <TextInput style={s.input} placeholder="tu_nombre" placeholderTextColor={colors.textDim}
              ...
              <View style={s.field}>
                ...
                <View style={s.genderGrid}>
                  {GENDERS.map(g => (
                    <TouchableOpacity key={g.key} style={[s.genderBtn, gender === g.key && s.genderBtnActive]} onPress={() => setGender(g.key)}>
                      <Text style={[s.genderTxt, gender === g.key && s.genderTxtActive]}>{g.label}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
          ...
          <View style={s.field}>
            ...
            <TextInput style={s.input} placeholder="usuario@ejemplo.com" placeholderTextColor={colors.textDim}
              value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" />
          </View>

          <View style={s.field}>
            <Text style={s.label}>CONTRASEÑA</Text>
            <TextInput style={s.input} placeholder="••••••••" placeholderTextColor={colors.textDim}
              value={password} onChangeText={setPassword} secureTextEntry />
          </View>

          {!isRegister && <Text style={s.forgot}>¿Olvidaste tu contraseña?</Text>}

          <TouchableOpacity onPress={handleSubmit} disabled={isLoading} activeOpacity={0.85}>
            <LinearGradient colors={['#006b63','#00c4b4','#00e5cc']} start={{x:0,y:0}} end={{x:1,y:0}} style={s.btnEnter}>
              <Text style={s.btnTxt}>{isLoading ? '...' : isRegister ? 'CREAR CUENTA' : 'ENTRAR'}</Text>
            </LinearGradient>
          </TouchableOpacity>
            ...
            <>
              ...
              <TouchableOpacity style={s.googleBtn} onPress={handleGoogleLogin} disabled={googleLoading} activeOpacity={0.85}>
                <View style={s.googleBtnInner}>
                  ...
                  <Text style={s.googleTxt}>{googleLoading ? 'Conectando...' : 'Continuar con Google'}</Text>
                </View>
              </TouchableOpacity>
            </>
          )}
...
const s = StyleSheet.create({
  ...
  glowTop: { position: 'absolute', width: 300, height: 300, borderRadius: 150, backgroundColor: 'rgba(0,110,130,0.08)', top: -100, left: -80 },
  glowBot: { position: 'absolute', width: 280, height: 280, borderRadius: 140, backgroundColor: 'rgba(40,0,120,0.07)', bottom: -80, right: -60 },

  logoArea: { alignItems: 'center', marginBottom: 32, position: 'relative' },
  ring1: { position: 'absolute', width: 90, height: 90, borderRadius: 45, borderWidth: 1, borderColor: 'rgba(0,229,204,0.12)', top: -5, alignSelf: 'center' },
  ring2: { position: 'absolute', width: 110, height: 110, borderRadius: 55, borderWidth: 1, borderColor: 'rgba(41,121,255,0.08)', top: -15, alignSelf: 'center' },
  ring3: { position: 'absolute', width: 132, height: 132, borderRadius: 66, borderWidth: 1, borderColor: 'rgba(217,70,239,0.05)', top: -26, alignSelf: 'center' },
  ...
  card:      { width: '100%', backgroundColor: colors.card, borderRadius: 24, borderWidth: 1, borderColor: colors.borderC, padding: 24 },
  ...
  avatarEmpty:    { width: 80, height: 80, borderRadius: 40, borderWidth: 2, borderColor: colors.borderC, borderStyle: 'dashed', backgroundColor: 'rgba(0,229,204,0.05)', alignItems: 'center', justifyContent: 'center' },
  ...
  avatarEditBadge:{ position: 'absolute', bottom: 0, right: 0, backgroundColor: colors.deep, borderRadius: 10, borderWidth: 1, borderColor: colors.borderC, width: 22, height: 22, alignItems: 'center', justifyContent: 'center' },

  field:  { marginBottom: 14 },
  label:  { fontSize: 10, letterSpacing: 2, color: colors.textDim, marginBottom: 6 },
  input:  { backgroundColor: 'rgba(8,20,36,0.95)', borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 14, color: colors.textHi, fontSize: 14 },

  genderGrid:      { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  genderBtn:       { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, borderWidth: 1, borderColor: colors.border, backgroundColor: 'rgba(8,20,36,0.95)' },
  ...
  googleBtn:      { borderRadius: 14, borderWidth: 1, borderColor: colors.border, backgroundColor: 'rgba(255,255,255,0.04)', overflow: 'hidden' },
  googleBtnInner: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 14, gap: 12 },
  googleG:        { fontSize: 18, fontWeight: '900', color: '#fff' },
  googleTxt:      { color: colors.textMid, fontSize: 14, fontWeight: '600' },
